...
import { EstimateLineItem, Estimate } from '@/src/modules/estimates/types';
import {
...
export default function CreateEstimateScreen() {
  ...
  const updateLineItem = useCallback(
    (id: string, field: keyof EstimateLineItem, value: string) => {
      setLineItems((prev) =>
        prev.map((item) => {
  ...
  const handleSave = useCallback(
    (asDraft: boolean) => {
      ...
      Alert.alert(
        'Success',
        asDraft ? 'Estimate saved as draft.' : 'Estimate created and sent.',
        [{ text: 'OK', onPress: () => router.back() }]
      );
    },
    [validate, estimates.length, clientName, clientEmail, lineItems, subtotal, taxAmount, discountNum, total, notes, addEstimate, router]
  );
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <View style={styles.card}>
              ...
              <Input
                label="Notes"
                placeholder="Scope details, terms..."
                value={notes}
...
const styles = StyleSheet.create({
  ...
  addItemText: { fontSize: 14, fontWeight: '500' as const, color: Colors.accent },
  errorText: { fontSize: 12, color: Colors.error, paddingHorizontal: Spacing.lg, marginBottom: Spacing.sm },
  lineItemCard: {
    backgroundColor: Colors.surface,
    borderRadius: BorderRadius.lg,
    padding: Spacing.lg,
  ...
  lineItemHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.md },
  lineItemLabel: { fontSize: 14, fontWeight: '600' as const, color: Colors.textSecondary },
  lineItemRow: { flexDirection: 'row', gap: Spacing.md },
  lineItemField: { flex: 1 },
  lineItemAmount: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  lineItemAmountLabel: { fontSize: 12, color: Colors.textSecondary, marginBottom: 4 },
  lineItemAmountValue: { fontSize: 16, fontWeight: '700' as const, color: Colors.text },
  totalsCard: {
    backgroundColor: Colors.surface,
    borderRadius: BorderRadius.lg,
    padding: Spacing.lg,
